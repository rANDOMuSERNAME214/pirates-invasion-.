...
function draw() {
  ...
  for (var i = 0; i < balls.length; i++) {
    ...
    for(var j = 0;j<boats.length;j++){
      var qwerty=Matter.SAT.collides(balls[i].body,boats[j].body)
      if(qwerty.collided){
        boats[j].remove(j)
...
function showBoats(){
  if(boats.length>0){
    if(boats.length<4 && boats[boats.length-1].body.position.x<width-300){
      var positions=[-130,-120,-110,-100]
      var boat=new Boat(width,height,height-100,200,200,random(positions))
      boats.push(boat)
    }
